import type { Order, RecentOrdersRequest } from "@/types";
import { createProductsListMock } from "./products.mocks";
import { quickSort, randomInt } from "@/utils";

const products = createProductsListMock();

export const createOrderMock = (data?: Partial<Order>): Order => {
...
export const createOrdersMock = (): Order[] => [
  createOrderMock({
    number: "876364",
    product: products[2],
    totalOrder: 325,
...
export const createRecentOrdersMock = ({
  ...
  limit = 4,
  sortBy,
}: RecentOrdersRequest): Order[] => {
  const orders = createOrdersMock().slice(skip, skip + limit);
